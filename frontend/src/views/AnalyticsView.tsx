...
export function AnalyticsView({
  ...
  // Calculate top OS from analytics
  const getTopOS = () => {
    if (!analytics?.clicks_by_os || analytics.clicks_by_os.length === 0) {
      return [];
    }
    const total = analytics.clicks_by_os.reduce((sum, o) => sum + o.clicks, 0);
    return analytics.clicks_by_os
      .map((item) => ({
        os: item.os || "Unknown",
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="flex items-center bg-white border-2 border-zinc-200">
            {(["24h", "7d", "30d"] as Period[]).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-4 py-2 text-xs hover:cursor-pointer font-bold uppercase border-r border-zinc-200 last:border-r-0 transition-colors ${
                  period === p
                    ? "bg-black text-white"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-[-1px] bg-zinc-200 border border-zinc-200 mb-12">
        <BentoItem
          title="Total Clicks"
          value={
            isLoading ? "..." : analytics?.total_clicks?.toLocaleString() || "0"
          }
          icon={MousePointer}
        />
        <BentoItem
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div className="lg:col-span-2 bg-white border-2 border-zinc-200 p-8">
          ...
          <BarChart data={analytics?.clicks_by_date} isLoading={isLoading} />
        </div>
                ...
                {getTopOS().map((item, i) => (
                  <div key={i} className="group">
                    <div className="flex justify-between text-sm font-mono mb-2">
                      <span className="text-zinc-400">{item.os}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white border-2 border-zinc-200 p-8">
          ...
          ) : analytics?.clicks_by_device &&
            analytics.clicks_by_device.length > 0 ? (
            <div className="space-y-4">
              {analytics.clicks_by_device.map((item, i) => {
                const total = analytics.clicks_by_device.reduce(
                  (sum, d) => sum + d.clicks,
                  0,
                );
                const percentage =
                  total > 0 ? Math.round((item.clicks / total) * 100) : 0;
        ...
        <div className="mt-8 bg-white border-2 border-zinc-200 p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {analytics.clicks_by_os.map((item, i) => {
              const total = analytics.clicks_by_os.reduce(
                (sum, o) => sum + o.clicks,
                0,
              );
              const percentage =
